...
if road == "Flexible Pavement":
    ...
    col1.markdown(f'<div class="metric-box bg1">SN Required<br>{SN_req:.3f}</div>',unsafe_allow_html=True)
    col2.markdown(f'<div class="metric-box bg2">SN Provided<br>{SN:.3f}</div>',unsafe_allow_html=True)
    col3.markdown(f'<div class="metric-box bg3">Total Thickness<br>{total:.1f} cm</div>',unsafe_allow_html=True)
    col4.markdown(f'<div class="metric-box bg4">W18<br>{W18:,.0f}</div>',unsafe_allow_html=True)
    ...
    layers = [d1,d2,d3]
    names = ["AC","Base","Subbase"]
    colors = ["#333333","#8c8c8c","#87CEEB"]
...
if road == "Rigid Pavement":
    ...
    with c1:
        D = st.number_input("D Slab Concrete (cm)",value=25.0)

    with c2:
        Db = st.number_input("Base (cm)",value=10.0)

    # simplified rigid calc
    D_req = (np.log10(W18)+1)*5
    ...
    col1.markdown(f'<div class="metric-box bg1">Required<br>{D_req:.1f} cm</div>',unsafe_allow_html=True)
    col2.markdown(f'<div class="metric-box bg2">Provided<br>{D:.1f} cm</div>',unsafe_allow_html=True)
    col3.markdown(f'<div class="metric-box bg3">Total Thickness<br>{total:.1f} cm</div>',unsafe_allow_html=True)
    col4.markdown(f'<div class="metric-box bg4">W18<br>{W18:,.0f}</div>',unsafe_allow_html=True)
    ...
    st.subheader("Rigid Pavement Cross Section")

    fig, ax = plt.subplots(figsize=(3,6))

    layers=[D,Db]
    names=["Concrete Slab","Base"]
